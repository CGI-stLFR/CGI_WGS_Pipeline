...
if($hg =~ /hg19/i){
	open DEPTH,"$Bin/bam2depth -q $basethres -Q $mapQthres $bam -b $Bin/hg19/nonN_region | " or die;
}
elsif($hg =~ /hg18/i)
{
	open DEPTH,"$Bin/bam2depth -q $basethres -Q $mapQthres $bam -b $Bin/hg18/nonN_region | " or die;
}
else
{
	open DEPTH,"$Bin/bam2depth -q $basethres -Q $mapQthres $bam |" or die;
}
...
Rline
	...
	system("../BIN/R-3.2.1/bin/R CMD BATCH  $figFile.R");
}
