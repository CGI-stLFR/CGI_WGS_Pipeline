    ...
    with open(vcf_file,'r') as infile:
        for line in infile:
            ...
            if CHROM == None:
                ...
                print("ERROR: VCFs should contain one chromosome per file", file=sys.stderr)
        ...
        for line in hbf:
            ...
            if len(el) >= 5:
                ...
                if chrom != CHROM and chrom.lstrip('chr') != CHROM.lstrip('chr'):
                    print("ERROR: Chromosome in haplotype block file doesn't match chromosome in VCF", file=sys.stderr)
                    print("Haplotype block file: " + hapblock_file, file=sys.stderr)
                    print("VCF file:             " + vcf_file, file=sys.stderr)
                    exit(1)
...
def parse_vcf_phase(vcf_file, CHROM, indels = False):
    ...
    with open(vcf_file, 'r') as vcf:
        ...
        for line in vcf:
            ...
            if len(el) < 10:
                continue
            if len(el) != 10:
                print("VCF file must be single-sample.", file=sys.stderr)
                exit(1)
    ...
    if PS_index == None:
        print("WARNING: PS flag is missing from VCF. Assuming that all phased variants are in the same phase block.", file=sys.stderr)
    ...
    with open(vcf_file, 'r') as vcf:
        ...
        for line in vcf:
            ...
            if chrom != CHROM and chrom.lstrip('chr') != CHROM.lstrip('chr'):
                print("ERROR: Chromosome in reference haplotype VCF doesn't match chromosome in VCF used for phasing", file=sys.stderr)
                print("reference haplotype VCF: " + vcf_file, file=sys.stderr)
                print("{} != {}".format(CHROM, chrom), file=sys.stderr)
                exit(1)
...
def get_ref_name(vcf_file):
    ...
    # default
    raise Exception("No ref name found for file: {}".format(vcf_file))
...
def hapblock_vcf_error_rate_multiple(assembly_files, vcf_files, phased_vcf_files, indels, largest_blk_only=False):
    ...
    for assembly_file, vcf_file, phased_vcf_file in zip(assembly_files, vcf_files, phased_vcf_files):
        try:
            ind_err = hapblock_vcf_error_rate(assembly_file, vcf_file, phased_vcf_file, indels)
            err += ind_err
...
def error_rate_calc(t_blocklist, a_blocklist, vcf_file, indels=False, phase_set=None):
    ...
    if different_alleles > 0:
        print("WARNING: {} positions had different ref,alt pairs and were skipped.".format(different_alleles), file=sys.stderr)
