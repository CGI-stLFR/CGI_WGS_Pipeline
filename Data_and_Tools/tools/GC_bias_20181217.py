...
class GC_Bias(object):
    ...
    def plot(self):
        ...
        for read_percs, ref_percs, gc_percs, norm_covs, read_length in zip(self.read_percs_list, self.ref_percs_list, self.gc_percs_list,
            ...
            fig, ax = plt.subplots()
            plt.plot(range(read_length + 1), read_percs, label='Read', color='r')
            plt.plot(range(read_length + 1), ref_percs, label='Reference', color='b')
            ax.grid(linestyle=':')
            ...
            if rix == 3:
                ...
                fig, ax = plt.subplots()
                plt.plot(range(read_length + 1), self.read_percs_list[0], label='Read1', color='r')
                plt.plot(range(read_length + 1), ref_percs, label='Reference', color='b')
                plt.plot(range(read_length + 1), self.read_percs_list[1], label='Read2', color='y')
                ax.grid(linestyle=':')
                plt.xlabel('GC Count (bp)')
                plt.ylabel('Frequency (%)')
                plt.legend()
...
def main():
    ...
    if not args:
        ...
        print >>sys.stderr, "\nERROR: Read file(s) must be specified!"
        sys.exit(1)
    else:
        data_files = args
